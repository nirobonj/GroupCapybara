    ...
    <div class="container">
        <div class="header">
            ...
            <div class="search-container" style="display: flex; align-items: center;">
                <form method="GET" action="{{ route('recomentShops') }}" style="display: flex; flex-grow: 1;">
                    <input type="text" name="search" placeholder="ค้นหาร้านทำเล็บ..." value="{{ request('search') }}" style="flex: 1; margin-right: 10px;">
        ...
        <div class="container">
            <div class="row">
                @if($shops->isEmpty())
                    <p>ไม่พบร้านค้าที่ตรงตามการค้นหา</p>
                @else
                    @foreach ($shops as $shop)
                        <div class="col-12 mb-4">
                            <div class="card h-100" style="background-color: #ffffff; border-radius: 10px; box-shadow: 0px 4px 6px rgba(0, 0, 0, 0.1);">
                                <div class="card-body">
                                    <div class="row">
                                        <div class="col-3 d-flex align-items-center justify-content-center">
                                            <div style="background-color: #f6a5ec; width: 100%; height: 120px; display: flex; align-items: center; justify-content: center; border-radius: 10px;">
                                                <img src="{{ asset('images/' . $shop->images_name) }}" alt="Shop Image" class="card-image">
                                            </div>
                                        </div>
                                        <div class="col-9">
                                            <a href="{{ route('shopDetail', ['id' => $shop->shop_id]) }}" style="text-decoration: none;">
                                            ...
                                            <div class="rating mb-2">
                                                ...
                                                @endfor
                                                <span style="color: black;">{{ number_format($averageRating, 1) }} ({{ $reviewCount }} รีวิว)</span>
                                            </div>
                                            @if (!empty($shop->promotion_detail) && $shop->promotion_detail !== '-')
                                                <a href="{{ route('show_promotion', ['shop_id' => $shop->shop_id]) }}" class="btn btn-warning">promotion</a>
                                            @endif
